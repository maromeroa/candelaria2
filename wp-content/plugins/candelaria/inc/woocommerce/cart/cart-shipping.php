		...
		<?php if ( $available_methods ) : ?>
			...
			<?php if ( is_cart() ) : ?>
				<p class="woocommerce-shipping-destination">
					<?php
					if(WC()->session->get('chosen_shipping_methods')[0]=='shipit_shipping_method'){
						$shipit = new Shipit();
						$communes = $shipit->communes();
						$c = [];
						foreach($communes as $key=>$val ){
							$c[$val->id] = $val->name;
						}
						if((WC()->session->get( 'shipping_commune'))!==null){
							$id = WC()->session->get( 'shipping_commune');
							$commune = "la comuna ".$c[$id];
							printf( esc_html__( 'Estimate for %s.', 'woocommerce' ) . ' ', '<strong>' . $commune . '</strong>' );
						}
						else{
							printf( esc_html__( 'Debes seleccionar una comuna.', 'woocommerce' ) . ' ', '<strong>' . '' . '</strong>' );
						}
					}
					else{
