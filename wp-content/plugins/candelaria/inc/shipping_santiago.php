   ...
   if ( ! class_exists( 'Candelaria_Santiago_Shipping_Method' ) ) {
     class Candelaria_Santiago_Shipping_Method extends WC_Shipping_Method {
       ...
       public function __construct() {
         ...
         $this->title              = "Despacho en provincia de Santiago"; // This can be added as an setting but for this example its forced.
       ...
       public function calculate_shipping( $package = array() ) {
		 ...
		 if($current_area!=''){
			 $comunas = ["Santiago","Huechuraba","Independencia","Quilicura","Recoleta","Renca","Las Condes","Lo Barnechea","Providencia","Vitacura","La Reina","Macul","Ñuñoa","La Florida","La Granja","El Bosque","La Cisterna","La Pintana","San Ramón","Lo Espejo","Pedro Aguirre Cerda","San Joaquín","San Miguel","Cerrillos","Estación Central","Maipú","Cerro Navia","Lo Prado","Pudahuel","Quinta Normal"];
			 $label = "(".$comunas[$current_area-1].")";
 ...
 ?>
   <p class="form-row form-row-wide" id="calc_shipping_location_field">
     ...
     <select name="calc_shipping_location" id="calc_shipping_location" rel="calc_shipping_location">
       ...
       <option value="2" <?php selected( $current_area, 2 ); ?>>Huechuraba</option>
       <option value="3" <?php selected( $current_area, 3 ); ?>>Independencia</option>
       <option value="4" <?php selected( $current_area, 4 ); ?>>Quilicura</option>
       <option value="5" <?php selected( $current_area, 5 ); ?>>Recoleta</option>
       <option value="6" <?php selected( $current_area, 6 ); ?>>Renca</option>
       <option value="7" <?php selected( $current_area, 7 ); ?>>Las Condes</option>
       <option value="8" <?php selected( $current_area, 8 ); ?>>Lo Barnechea</option>
       <option value="9" <?php selected( $current_area, 9 ); ?>>Providencia</option>
       <option value="10" <?php selected( $current_area, 10 ); ?>>Vitacura</option>
       <option value="11" <?php selected( $current_area, 11 ); ?>>La Reina</option>
       <option value="12" <?php selected( $current_area, 12 ); ?>>Macul</option>
       <option value="13" <?php selected( $current_area, 13 ); ?>>Ñuñoa</option>
       <option value="14" <?php selected( $current_area, 14 ); ?>>La Florida</option>
       <option value="15" <?php selected( $current_area, 15 ); ?>>La Granja</option>
       <option value="16" <?php selected( $current_area, 16 ); ?>>El Bosque</option>
       <option value="17" <?php selected( $current_area, 17 ); ?>>La Cisterna</option>
       <option value="18" <?php selected( $current_area, 18 ); ?>>La Pintana</option>
       <option value="19" <?php selected( $current_area, 19 ); ?>>San Ramón</option>
       <option value="20" <?php selected( $current_area, 20 ); ?>>Lo Espejo</option>
       <option value="21" <?php selected( $current_area, 21 ); ?>>Pedro Aguirre Cerda</option>
       <option value="22" <?php selected( $current_area, 22 ); ?>>San Joaquín</option>
       <option value="23" <?php selected( $current_area, 23 ); ?>>San Miguel</option>
       <option value="24" <?php selected( $current_area, 24 ); ?>>Cerrillos</option>
       <option value="25" <?php selected( $current_area, 25 ); ?>>Estación Central</option>
       <option value="26" <?php selected( $current_area, 26 ); ?>>Maipú</option>
       <option value="27" <?php selected( $current_area, 27 ); ?>>Cerro Navia</option>
       <option value="28" <?php selected( $current_area, 28 ); ?>>Lo Prado</option>
       <option value="29" <?php selected( $current_area, 29 ); ?>>Pudahuel</option>
       <option value="30" <?php selected( $current_area, 30 ); ?>>Quinta Normal</option>
     </select>
	</p>
   <?php
}

add_action('woocommerce_candelaria_form','candelaria_santiago_calculate_form');
...
function woocomerce_text_strings( $translated_text, $text, $domain ) {            
       switch ( $translated_text ) {            
            case 'shipping calculator' :        
                $translated_text = "Elige una comuna";        
                break;    
        }        
